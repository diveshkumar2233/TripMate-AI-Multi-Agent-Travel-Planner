...
function showApproval(data) {
  ...
  approvalRequest.textContent = data.approval_request ||
    "A draft is ready. Approve it or provide revision feedback to improve the trip plan.";
  section.classList.remove("hidden");
...
async function sendMessage() {
  ...
  if (waitingForApproval) {
    showError("Please approve or revise the current draft before starting a new plan.");
    return;
  }
  ...
  try {
    ...
    if (data.requires_approval) {
      ...
      hideApproval();
      showResult(data.answer || data.itinerary || "No content returned.", data.thread_id, false);
    }
  } catch (error) {
...
async function submitApproval(approved) {
  ...
  if (!currentThreadId) {
    showError("No active thread found to approve.");
    return;
  }
  ...
  if (!approved && !feedback) {
    showError("Please add revision feedback before requesting changes.");
    feedbackInput.focus();
    return;
  }
