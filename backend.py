...
def _safe_llm_invoke(messages: list, fallback_text: str) -> AIMessage:
    """
    Wraps llm.invoke() so a Groq rate-limit (429) or transient API error
    doesn't crash the whole graph run. Always returns an AIMessage.
    """
    try:
        return llm.invoke(messages)
    except RateLimitError as exc:
        print(f"GROQ RATE LIMIT HIT: {exc}", flush=True)
        return AIMessage(
            content=(
                f"{fallback_text}\n\n"
                "_(Note: the AI model hit its daily Groq rate limit. This "
                "section used a fallback message — try again later, or "
                "upgrade your Groq plan for higher limits: "
                "https://console.groq.com/settings/billing)_"
            )
        )
        ...
        return AIMessage(
            content=(
                f"{fallback_text}\n\n"
                f"_(Note: the AI service returned an error: {exc}.)_"
            )
        )
...
def _llm_text(system_prompt: str, user_prompt: str) -> str:
    response = _safe_llm_invoke(
        ...
        fallback_text="Unable to generate this section right now.",
    )
...
def flight_agent(state: TravelState):
    ...
    try:
        ...
        response = _safe_llm_invoke(
            ...
            fallback_text="Flight information is temporarily unavailable.",
        )
...
"""
    ...
    response = _safe_llm_invoke(
        ...
        fallback_text="Budget analysis is temporarily unavailable.",
    )
...
"""
    ...
    response = _safe_llm_invoke(
        ...
        fallback_text=(
            "A draft itinerary could not be generated right now due to an AI "
...
"""
    ...
    response = _safe_llm_invoke(
        ...
        fallback_text=(
            "The final travel plan could not be generated right now due to an "
            "AI service issue. Please try again shortly. In the meantime, here "
            "is what was gathered so far."
        ),
    )
